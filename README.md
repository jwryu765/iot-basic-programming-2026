...
1. **포인터**
    ...
    - 포인터 이해 : [확인](./Day05/Chap09-Solution/Chap09-02-app/main.c)
        - 변수의 주소(메모리상)를 담고 감접참조연산자(*)로 그 변수의 값을 사용하기 위한 것
...
5. Tip
    - #pragma region, #pragma end region : 소스코드 영역에서 일부내용 축소시키고자 할때
...
1. 함수 포인터 : [확인](./Day07/Chap15-Solution/Chap15-01-app/main.c)
...
2. void 포인터 : [확인](./Day07/Chap15-Solution/Chap15-03-app/main.c)
    ...
    - 포인터 특징 - int 형 int*, double형 double* 와 같이 타입형이 일치해야
...
3. 메모리 동적할당
    ...
    - `calloc` - contiguos allocation. 연속된 메모리 동적할당. 0 초기화, malloc보다 느림 : [확인](./Day07/Chap16-Solution/Chap16-03-app/main.c)
    - `realloc` - re-allocation. 메모리 크기 조정
    - `free` - 메모리 반환
    - **NULL** - 아무것도 카리키지 않는 포인터값!
...
5. 동적 할당 저장공간 활용 : [확인](./Day07/Chap16-Solution/Chap16-04-app/main.c)
    - 영어사전과 같은 데이터를 저장할때 : [확인](./Day07/Chap16-Solution/Chap16-05-app/main.c)
        - 배열로 처리하면 메모리 낭비가 심함
        - 동적할당으로 메모리 절약
...
1. 파일 확장명(확장자) 확인
    ...
    - 윈도우 탐색기 > `보기` > `표시` > `파일확장명` 활성화

2. 폴더 및 파일경로
    - 정대경로 - `"C:\DEV\IDE\Microsoft VS Code\591199df40\resources\app\LICENSE.rtf"` 와 같이. 드라으브부터 전체 경로를 모두 기재
        - 장점 - 정확한 경로가 표기
        - 단점 - 경로 문자열이 아주 길어질 수 있음
    - 상대경로  - `\LICENSE. rtf`와 같이, .(현재폴더_, ..(부모폴더) 기호를 사용해서 짧게 경로를 표시 방법)
        - 장점 - 경로를 짧게 표기.
        - 단점 - 개발자 직접경로가 어떤식으로 구성되는지 파악필요. 같이 폴더에 존재하는 경우 사용 가능

3. 파일 입출력 : [확인](./Day09/Chap18-Solution/Chap18-01-app/main.c)
    - 키보드 등 입력은 버퍼를 통한 입력 스트림(stream)에, 출력은 출력 스트림(stream)에 보관 되어 있음
    - 파일, 콘솔 등에 전달
    ...
    - fprintf() - 포맷에 맟줘서 파일에 쓰기 함수
    - stdio.h - stdin(표준입력), stdout(표준출력), stderr(표준에러) 포함
    - 인코딩 방식 - `EUC-KR`(CP-949, 멀티바이트, 한글만표현, 2byte), UTF-8(3byte 한글자 최대 6byte. 한글은 3byte)
...
7. 전처리 지시자 : [확인](./Day09/Chap19-Solution/Chap19-01-app/main.c)
    ...
    - #define - 새로 정의하는 지시자. 자주 사용될 상수값을 정의할 때 많이 . 주로 대문자로 지정
